...
What exists in the tree is tracked in [10-engine-progress.md](10-engine-progress.md) (living) and in [`docs/`](../docs/index.md). In the tree: window port, clock, `ScreenStack`, `InputMapper` / `Action`, `MainMenuScreen`, empty `GameplayScreen`. There is no pause overlay or World.
...
- Tests: [`tests/unit_tests/`](../tests/unit_tests/) (`example_test`, `game_test`, `fixed_timestep_test`, `screen_stack_test`, `menu_gameplay_test`, `input_mapper_test`). Windowless.
