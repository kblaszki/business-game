...
**In the tree today:** [10-engine-progress.md](10-engine-progress.md) (screens, pause, `World` / Sandbox dummy). This chapter stays prospective.

The loop in `Game::run()` (Closed, focus, mapper, fixed tick, `ScreenStack`, `DESIGN_SIZE`) is the starting point to extend. Later phases of this chapter are genre / extra levels. Debug tests register windowless suites via `add_unit_test` in [`tests/unit_tests/CMakeLists.txt`](../tests/unit_tests/CMakeLists.txt). SFML Audio and Network stay off ([`cmake/FetchSFML.cmake`](../cmake/FetchSFML.cmake)).
