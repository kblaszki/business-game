...
Product direction is still open. Do not assume a board-game or business sim exists — add those explicitly when asked. Engine ports from [mvp/10-v0.1-interface-base.md](../../mvp/10-v0.1-interface-base.md) that have landed: `WindowI` / `DrawerI`, `ScreenI` / `ScreenUpdaterI` (pause cuts paddle keys), `EntityI` / `CollidableI` (`World` holds `unique_ptr<EntityI>`), and menu `Button` + `HitTestI` (no global event managers). The playable demo stays Arkanoid.
