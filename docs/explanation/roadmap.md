...
A C++23 SFML 3.1 breakout on the landed engine split (`engine/` / `games/arkanoid/`): Start/Quit menu, three stages (Stage1 → Stage2 → Stage3), four power-ups (Wide, MultiBall, Slow, ExtraLife), pause overlay via `SceneStack`, headless tests. Legacy `src/` is removed. The tree still spells namespace `eng` until the Phase II rename.
