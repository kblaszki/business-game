...
| `EntityI` | `fixedUpdate`, `draw`, `alive()` (default true). No pose. Menu `Button` implements this plus `HitTestI`. |
...
There is no score, font HUD, brick HP, deferred spawn, or `Action::MoveDummy`. Paddle hold is `GameplayScreen::handleEvent` Left/Right. Menu `Button`s live on `MainMenuScreen`, not in `World`.
