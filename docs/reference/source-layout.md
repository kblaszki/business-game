...
related_docs:
  ...
  - ../../mvp/README.md
keywords: [layout, directories, gameLib, game, targets, cmake sources, ScreenStack, IDrawer]
...
| `src/IScreen.hpp` / `src/IDrawer.hpp` / `src/ScreenStack.*` / `src/*Screen.*` / `src/Action.hpp` / `src/InputMapper.*` | Screens, draw port, and consume — see [screens-and-input.md](screens-and-input.md) |
...
| `src/IDrawer.hpp` / `src/SfmlDrawer.*` | Draw port (`draw(sf::Drawable&)`) and SFML adapter wrapping `sf::RenderTarget` |
| `tests/unit_tests/` | GoogleTest suites (Debug; no `sf::RenderWindow`) |
| `tests/unit_tests/fakes/` | `SpyScreen` (no `ScreenStack` include), `DrawerMock`, `NullDrawer` |
