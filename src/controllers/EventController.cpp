...
void EventController::registerEventHandler(const sf::Event::EventType eventType,
                                           EventHandler&& handler)
{
    if(eventHandlers.cend() not_eq eventHandlers.find(eventType))
    {
        throw std::runtime_error{"Can't register event:" + to_string(eventType)
                                 + " handler, because it already exists!"};
    }
    eventHandlers.emplace(eventType, std::move(handler));
}

void EventController::unregisterEventHandler(
    const sf::Event::EventType eventType)
{
    if(eventHandlers.cend() == eventHandlers.find(eventType))
    {
        throw std::runtime_error{"Can't unregister event:"
                                 + to_string(eventType)
                                 + " handler, because it not exists!"};
    }
