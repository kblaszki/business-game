...
- [ ] Same-change Diátaxis docs for every touched src/ path (not only a final docs wave)
...
Each agent: owned paths only; list new `src/*.cpp` in [`src/CMakeLists.txt`](../../../src/CMakeLists.txt) (`gameLib`); register new tests with `add_unit_test`; update Diátaxis docs whose `related_code` lists files they changed. **Agents do not `git commit`** (they would race). The orchestrator commits after stitch.
...
3. **Same-change docs** (required when code changed, not only at the end):
   ...
   - Invoke [update-docs](../update-docs/SKILL.md) when the wave changed code.
